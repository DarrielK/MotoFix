        ...
        <aside class="sticky top-0 z-[20] flex flex-col items-center justify-between w-[5%] h-screen bg-black py-6">
            ...
            <div class="flex flex-col items-center gap-6">
                ...
                <a href="{{ route('gudang.laporan.barang-masuk') }}">
                    <svg data-tooltip-target="tooltip-brg-masuk" data-tooltip-placement="right" class="w-10 h-10 p-2 rounded-md bg-gray-200 hover:bg-gray-300 text-gray-500 hover:text-gray-600" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 -960 960 960" fill="currentColor">
                                ...
                                <ul class="space-y-1 text-sm text-gray-700 capitalize">
                                    <li>
                                        <div class="flex items-center p-2 rounded-lg hover:bg-gray-300">
                                            <input type="radio" value="" name="filter" class="w-4 h-4" onchange="document.getElementById('filterForm').submit()" {{ request('filter') == '' ? 'checked' : '' }}>
                                            <label class="ms-2 text-sm font-medium">All</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="flex items-center p-2 rounded-lg hover:bg-gray-300">
                                            <input type="radio" value="7days" name="filter" class="w-4 h-4" onchange="document.getElementById('filterForm').submit()" {{ request('filter') == '7days' ? 'checked' : '' }}>
                                            <label class="ms-2 text-sm font-medium">Last 7 Days</label>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="flex items-center p-2 rounded-lg hover:bg-gray-300">
                                            <input type="radio" value="30days" name="filter" class="w-4 h-4" onchange="document.getElementById('filterForm').submit()" {{ request('filter') == '30days' ? 'checked' : '' }}>
                                            <label class="ms-2 text-sm font-medium">Last 30 Days</label>
                                        </div>
                                    </li>
                                </ul>
                            </div>
                        ...
                        <tbody>
                            @forelse($riwayat as $data)
                                <tr class="h-14 bg-gray-100 border-b border-gray-300 hover:bg-gray-200">
                                    ...
                                    <th class="px-4 py-2 font-normal capitalize">{{ $data->item->supplier ?? '-' }}</th>
                                    <th class="px-4 py-2 font-normal capitalize">{{ $data->item->category->name ?? '-' }}</th>
                                    <th class="px-4 py-2 text-green-500">+{{ $data->qty }}</th>
                                    <th class="px-4 py-2 font-normal capitalize">{{ $data->item->unit ?? '-' }}</th>
                                    <th class="px-4 py-2 font-normal">Rp{{ number_format($data->item->price, 0, ',', '.') }}</th>
                                    <th class="px-4 py-2 font-normal">{{ \Carbon\Carbon::parse($data->date)->format('d M Y') }}</th>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="8" class="text-center py-4 text-gray-500">Belum ada data barang masuk.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-gray-200 rounded-lg shadow-sm">
                ...
                <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-300">
                    ...
                    <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center" data-modal-toggle="filter-date-modal">
                ...
                <!-- Modal body -->
                <form class="px-4 pb-10" action="{{ route('gudang.laporan.barang-masuk') }}" method="GET">
                    ...
                    <div class="flex gap-4">
                        <a href="{{ route('gudang.laporan.barang-masuk') }}" type="submit" class="w-1/2 gap-2 text-white inline-flex items-center justify-center bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:outline-none focus:ring-yellow-300 font-medium rounded-lg text-sm py-2 text-center">
